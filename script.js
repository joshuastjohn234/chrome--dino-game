...
const scoreElem = document.querySelector("[date-score]")
const startStartElem = document.querySelector("[date-start-screen]")
...
function handleStart(){
    ...
    setupDino()
    setupCactus()
    startStartElem.classList.add("hide")
    window.requestAnimationFrame(update)
}
...
function handleLose() {
    ...
    setTimeout(() => {
        document.addEventListener("keydown", handleStart, { once: true})
        startStartElem.classList.remove("hide")
    }, 100)
}
